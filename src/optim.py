...
from dgl.distributed.optim.pytorch.utils import alltoall_cpu, alltoallv_cpu
...
class DistSparseGradOptimizer(abc.ABC):
    ...
    def step(self, hotness_list, threshold, from_top=False):
        """The step function.
            ...
            for emb in self._params:
                ...
                for trace in emb._trace:
                    if trace[1].grad is not None:
                        if hotness_list is not None:
                            if from_top:
                                bool_mask = hotness_list[trace[0]] <= threshold
                            else:
                                bool_mask = hotness_list[trace[0]] > threshold
                            idics.append(trace[0][bool_mask])
                            grads.append(trace[1].grad.data[bool_mask])
                        else:
...
class SparseAdam_with_mask(DistSparseGradOptimizer):
    ...
    def __init__(self, params, lr, betas=(0.9, 0.999), eps=1e-08):
        super(SparseAdam_with_mask, self).__init__(params, lr)
        self._eps = eps
